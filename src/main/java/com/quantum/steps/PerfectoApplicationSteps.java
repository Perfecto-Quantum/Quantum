 ...
 * applications, with cucumber steps annotations.
 * <p>
 * Contexts used for testing:
 * <ul>
 * <li><b>WEBVIEW</b> context is used to identify DOM based UI Elements.
 * <li><b>NATIVE_APP</b> context is used to identify UI Elements defined with
 * OS-based classes.
 * <li><b>VISUAL</b> context is used to identify UI Elements with Perfecto
 * visual analysis.
 * </ul>
 * <p>
...
public class PerfectoApplicationSteps {
	 ...
	 * @see <a href=
	 *      "https://community.perfectomobile.com/series/21760/posts/995065">Application
	 *      Identifier</a>
	 */
	...
	/**
	 * Cleans the data (including cache) from any application installed on the
	 * device and brings the application back to its initial state.
	 * 
	 * @param name the application name as it is displayed on the device screen
	 */
	 ...
	 * manager. Supported file formats include APK files for Android, IPA for iOS
	 * and COD for BlackBerry Devices.
	 * 
	...
	/**
	 * Installs a single application on the device, with sensor instrumentation and
	...
	/**
	 * Uninstalls all applications on the device, returning the device to its
	 * initial state. It does not affect applications pre-installed on the device.
	 */
	...
	/**
	 * Verifies the application version. The test will continue to run in case of
	 * failure.
	 * 
	 * @param version the application version to verify
	 * @return <code>true</code> if the version is verified, <code>false</code>
	 *         otherwise
	 */
	...
	/**
	 * Verifies the application orientation. The test will continue to run in case
	 * of failure.
	 * 
	 * @param orientation the application orientation to verify, landscape or
	 *                    portrait
	 * @return <code>true</code> if the orientation is verified, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * 
	 * @param orientation the application orientation to check, landscape or
	 *                    portrait
	 */
	 ...
	 * 
	 * Identify the object using the <i>Object Repository</i> or an XPath
	 * expression.
	 * 
	...
	/**
	 * Sets the text of a application element. Use the text parameter to specify the
	 * text to set.
	 *
	 * Identify the edit field object using the <i>Object Repository</i> or an XPath
	 * expression.
	...
	/**
	 * Verifies whether an element exists in the application. The test will continue
	 * to run in case of failure.
	 * 
	 * @param locator the object identifier
	...
	/**
	 * Checks whether an element exists in the application. Stops the test in case
	 * of failure.
	 * 
	 * @param locator the object identifier
	 */
	...
	/**
	 * Verifies that the text appears on the device screen, using visual analysis.
	 * The test will continue to run in case of failure.
	 * 
	...
	/**
	 * Checks that the text appears on the device screen, using visual analysis.
	 * Stops the test in case of failure.
	 * 
	...
	/**
	 * Verifies that the image appears on the device screen, using visual analysis.
	 * The test will continue to run in case of failure.
	 * 
	...
	/**
	 * Checks that the image appears on the device screen, using visual analysis.
	 * Stops the test in case of failure.
	 * <p>
	 ...
	 * manager.
	 * 
	 * @param img the repository path, including directory and file name, where to
	 *            locate the image
	 * @see <a href="https://community.perfectomobile.com/posts/912493">Perfecto Lab
	 *      Repository</a>
	 */
	 ...
	 * <p>
	 * Identify the element using the <i>Object Repository</i> or an XPath
	 * expression.
	 * 
	 * @param seconds the wait duration
	 * @param locator the object identifier
	 ...
	 * <p>
	 * This is done with a direct native command to the device OS, and not with
	 * navigation.
	 * 
	...
	/**
	 * Start image injection to the device camera to application using application
	 * name.
	 *
	 * @param repositoryFile the image repository file location
	 * @param name           the application name as it is displayed on the device
	 *                       screen
	 */
	...
	public static void startImageInjection(String repositoryFile, String id) {
		DeviceUtils.startApp(id, "identifier");
		// Change to app context after open app
		switchToContext("NATIVE_APP");
	...
	/**
	 * Send fingerprint with success result to application with the applicaiton
	 * name.
	 *
	 ...
	 *
	 * @param errorType indicates why the authentication failed. May be authFailed,
	 *                  userCancel, userFallback, systemCancel, or lockout
	 * @param id        the identifier of the application
	 */
	 ...
	 * 
	 * @param audioFilePath - example: PUBLIC:Audio\\play25s1.wav
	 */
	...
	/**
	 * This step will perform an audit of the accessibility features in the
	 * application. To check the entire application, this command needs to be
	 * repeated for each application screen.
	 *
